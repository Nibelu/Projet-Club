...
namespace Projet_WinForm
{
    class BDD
    {
        ...
        public List<villeAdh> SearchAdherentVille(string recherche, int idClub)
        {
            villeAdh lesAdherents = null;
            List<villeAdh> ListAdherent = new List<villeAdh>();
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                connection.Open();
                string query = "SELECT COUNT(*) as nb,villAdh FROM Adherent GROUP BY villeAdh ";
                //Create Command
                MySqlCommand cmd = new MySqlCommand(query, connection);
                //Create a data reader and Execute the command
                using (MySqlDataReader dataReader = cmd.ExecuteReader())
                {
                    ...
                    while (dataReader.Read())
                    {
                        lesAdherents = new villeAdh(                            
                            (string)dataReader["nomVille"],
                            (int)dataReader["nb"]);
